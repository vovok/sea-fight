...
class TournaimentStatistic(object):
	...
	def count_middles(self):
		...
		self.step_winners = [player.stat.step for player in self.players_copy_list if len(player.stat.ships_defeat) == 10]
		self.step_loosers = [player.stat.step for player in self.players_copy_list if len(player.stat.ships_defeat) != 10]
		self.scores_loosers = [player.stat.score for player in self.players_copy_list if len(player.stat.ships_defeat) != 10]
	...
	def startegy_effect(self):
		...
		for player in self.players_copy_list:
			...
			report_strategy[pl_stat].append([player.strategy.ships_strategy_collocation, player.strategy.steps_strategy])
		...
		if state == u'Попал!':
			...
			for ship in player2.ships:
				if crd in ship.cord:
					...
					self.steps_cords = filter(lambda x: x not in ship.halo and x not in self.alien_cords, self.steps_cords)
			self.recomendation_pool = []
			self.succ_shoots = []
...
if __name__ == '__main__':
	...
	STRATEGY_QUOTA = [y for y in ["for_1_ship_left",
	                             ...
	                             "for_1_ship_36",
	                             "random_12"] for x in range(turnaiment_player_counter/8)]
	...
	while len(tur_player_list) != 1:
		...
		info(u'Турнир выйграл: %s, набрал очков: %s', tur_player_list[0].player_name, tur_player_list[0].stat.tur_scores)
